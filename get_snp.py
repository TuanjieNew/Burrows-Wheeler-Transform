...
print("vcf file outputs to folder: output")
f_out.write('##fileformat=VCFv4.3\n')
f_out.write('##source=LambdaPhageV01\n')
f_out.write('##reference=/2_disk/niutj/python_pros/lambda_virus.fa\n')
...
for line in f_in:
    ...
    if int(ls[3])>depth:
        ...
        if frac>0.8:
            ...
            f_out.write(str(ls[0])+'\t'+str(ls[1])+'\t'+'.\t'+str(ls[2])+'\t'+alt[1]+'\t.\tPASS\t'+ info+'\n')
